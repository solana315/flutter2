...
class _PlanoTratamentoPageState extends State<PlanoTratamentoPage> {
					...
					child: ConstrainedBox(
						...
						child: FutureBuilder<Map<String, dynamic>>(
							...
							builder: (context, snapshot) {
								...
								final filtered = planos.where((p) => _matchesFilter(p, showActive)).toList();
								...
								return SingleChildScrollView(
									...
									child: Column(
										...
										children: [
											...
											_PlanDetails(primaryGold: primaryGold, detailsFuture: _detailsFuture),
	...
	static bool _matchesFilter(Map<String, dynamic> plan, bool showActive) {
		final status = (plan['status'] ?? plan['estado'] ?? '').toString().toLowerCase();
		final ativo = plan['ativo'];
		final isActive = (ativo is bool)
			? ativo
			: !(status.contains('concl') || status.contains('final') || status.contains('inativ'));
...
class _PlanCard extends StatelessWidget {
	...
	@override
	Widget build(BuildContext context) {
		final title = (plan['titulo'] ?? plan['nome'] ?? plan['name'] ?? 'Plano').toString();
		final doctor = (plan['doctor'] ?? plan['medico'] ?? plan['responsavel'] ?? '').toString();
		...
		return Container(
			...
			decoration: BoxDecoration(
				...
				border: Border.all(
					color: selected ? primaryGold.withAlpha((0.30 * 255).round()) : Colors.transparent,
				),
			),
			child: Material(
				color: Colors.white,
	...
	Widget build(BuildContext context) {
		return Container(
			...
			decoration: BoxDecoration(
				...
				borderRadius: BorderRadius.circular(12),
				boxShadow: [
					BoxShadow(color: const Color(0x0F000000), blurRadius: 8, offset: const Offset(0, 4)),
				],
			),
			child: Column(
				crossAxisAlignment: CrossAxisAlignment.stretch,
				children: [
					const Text('Detalhes do Plano', style: TextStyle(fontWeight: FontWeight.w600)),
					const SizedBox(height: 8),
					if (detailsFuture == null)
						const Text('Selecione um plano para ver detalhes.', style: TextStyle(color: Colors.black54))
					else
